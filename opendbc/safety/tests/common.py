...
class SafetyTest(SafetyTestBase):
  ...
  SCANNED_ADDRS = [*range(0x800),                      # Entire 11-bit CAN address space
                   ...
                   *range(0x6CD5554, 0x6CD5558),            # Honda Bosch radarless camera (LANE_PATH, HUD_OBJECTS)
                   0xF31AA54]                               # Honda Bosch radarless camera (LKAS_HUD_2)
        ...
        if isinstance(tc, type) and issubclass(tc, SafetyTest) and attr != current_test:
          ...
          if tx is not None and not attr.endswith('Base'):
            ...
            if attr.startswith('TestHonda'):
              ...
              tx = list(filter(lambda m: m[0] not in [0x1FA, 0x30C, 0x33D, 0x33DB, 0x1EF, 0x35E, 0x6CD5554, 0xF31AA54, 0x6CD5557], tx))
