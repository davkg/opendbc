...
#define FCA_GIORGIO_ACC_BUTTON      0x2FAU
...
static safety_config fca_giorgio_init(uint16_t param) {
  ...
  static RxCheck fca_giorgio_rx_checks[] = {
    {.msg = {{FCA_GIORGIO_ACC_1, 0, 8, 12U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true, .max_counter = 0U}, { 0 }, { 0 }}},
    {.msg = {{FCA_GIORGIO_ABS_1, 0, 8, 100U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true, .max_counter = 0U}, { 0 }, { 0 }}},
    {.msg = {{FCA_GIORGIO_ABS_3, 0, 8, 100U, .ignore_checksum = true, .ignore_counter = true, .ignore_quality_flag = true, .max_counter = 0U}, { 0 }, { 0 }}},
...
static void fca_giorgio_rx_hook(const CANPacket_t *msg) {
  if (msg->bus == 0U) {
    ...
    if (msg->addr == FCA_GIORGIO_ACC_1) {
      ...
      int acc_status = (msg->data[2] & 0xE0U) >> 5;
      bool cruise_engaged = (acc_status == 2) || (acc_status == 4);
      acc_main_on = cruise_engaged || (acc_status == 1) || (acc_status == 3) || (acc_status == 5);
